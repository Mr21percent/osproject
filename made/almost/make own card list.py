# -*- coding: utf-8 -*-
"""
Created on Wed Nov 20 17:00:39 2019
...
# start with we get mic form server and card_signal_data from other file

def making_mci_cardlist(server, mycard):
    list1=[]
    list2=[]
    for i in range(len(server)):
        for k in range(len(mycard)):
            if server[i][0]==mycard[k][0]:
                list1.append(server[i])
                list2.append(mycard[k])
    return list1, list2

mci, card_signal_data=making_mci_cardlist(server_card_info,card_signal_data)
print(mci)